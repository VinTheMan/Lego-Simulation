...
public class TransparentBackgroundScreenshotRecorder : MonoBehaviour {
    ...
    void LateUpdate()
    {
        ...
        else
        {
            ...
            // Debug.Break();
            Destroy(GetComponent<TransparentBackgroundScreenshotRecorder>());
            Destroy(gameObject) ;
        }
    }
    ...
    void SavePng()
    {
        ...
        CameraGetPrtSc cgps = FindObjectOfType<CameraGetPrtSc>() ;
        NetworkControl nc = FindObjectOfType<NetworkControl>() ;

        var pngShot = outputTexture.EncodeToPNG();
        int index = cgps.ChangeWaitAndGetIndex() ;
        nc.SavePicture(pngShot, Application.dataPath + "/Screenshot" + index + ".png", index );
